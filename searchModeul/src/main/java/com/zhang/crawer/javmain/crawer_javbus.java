...
public class crawer_javbus {
    public ArrayList<magnet_model> javbus(String url) throws IOException, URISyntaxException {
        ...
        setHeader.setUp();
        builder.host = "https://www.javbus.com/";
        ...
        try {
            ...
            for (Element element : elements) {
                ...
                //获取番号
                model.setNum(element.select("date").text().substring(0, element.select("date").text().indexOf(" ")));
                System.out.println(model.getNum());
                //获取标题

//                model.setTitle(element.select("span").text().substring(0,element.select("span").text().indexOf(" ")));
//                插入中文标题
                if (!fanyiV3Demo.tranStr(element.select("span").text().substring(0,element.select("span").text().indexOf(" "))).equals("0")){
                    model.setTitle(element.select("span").text().substring(0,element.select("span").text().indexOf(" "))+"\n"+fanyiV3Demo.tranStr(element.select("span").text().substring(0,element.select("span").text().indexOf(" "))));
                }else {
                    model.setTitle(element.select("span").text().substring(0,element.select("span").text().indexOf(" ")));
                }
                ...
                try {
                    ...
                    try {
                        ...
                        downloadImages(javdb_inDoc.getElementsByAttributeValue("class", "panel-block first-block").select("span").text(), coverImg);
